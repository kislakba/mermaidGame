...
import random
class LittleFish():
    ExposedEvent = pygame.event.Event(pygame.USEREVENT, attr1='LittleFishExposed')
    def __init__(self, screen, count):
        ...
        self.rectangle = pygame.rect.Rect(width + int(width / 20) / 2, self.y + int(height / 10) / 2, int(width / 30),
                                          int(height / 20))
        ...
        for i in range(0, 3):
            self.flyImages.append(
                pygame.transform.scale(pygame.image.load("images/f" + str(i) + ".png"),
                                       (self.rectangle[2], self.rectangle[3])))
        for i in range(0, 3):
            self.flyImagesLeft.append(
                pygame.transform.scale(pygame.image.load("images/fi" + str(i) + ".png"),
                                       (self.rectangle[2], self.rectangle[3])))
        self.exposed = False
